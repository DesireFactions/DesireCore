...
public class PaginatedMenu extends MenuHolder
{
    ...
    private PaginatedMenu(String title, int rows, int initial_pages)
    {
        ...
        if (initial_pages < 1) { throw new IllegalArgumentException("Cannot instantiate a PaginatedMenu with a nonpositive page count!"); }
    ...
    boolean isViewingAny(Player player)
    {
        for (Inventory inv : this.inventories)
        {
            ...
            if (inv.getViewers().contains(player)) { return true; }
        }
    ...
    {
        if ((page < 0) || (page >= this.getPageCount())) { return null; }
    ...
    {
        if (set_to.length != this.getMaxItems()) { return false; }
    ...
    {
        if (this.getMaxItems() != new_page.length) { return false; }
