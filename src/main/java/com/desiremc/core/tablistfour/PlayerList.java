...
{
    private static final Class<?> PACKET_PLAYER_INFO_CLASS = CustomPacketPlayOutPlayerInfo.class;
    ...
    /**
     * Due to the amount of times I have to check if a version is higher than 1.8, all reflection calls will be replace
     * with this method.
     * 
     * @param update
     * @return
    ...
    /**
     * Tries to return an existing table instance for a player. If one does not exist, it will create a new one with a
     * default size.
     * 
     * @param player
    ...
    /**
     * Resets a player's tablist. Use this if you have want the tablist to return to the base-minecraft tablist
     */
    ...
    /**
     * Clears all the values for a player's tablist. Use this whenever a player first joins if you want to create your
     * own tablist. This is here to remind you that you MUST call either this method or the "clearCustomTabs" method. If
     * you do not, the player will continue to see the custom tabs until they relog.
     */
    ...
    /**
     * This is used to create the table. If you want to create a custom tablist, then this should be called right after
     * the playlist instance has been created.
     */
    ...
    public static class ReflectionUtil
    {
        ...
        static Object invokeMethod(Object handle, String methodName,
...
/**
 * Stores information about a minecraft user's skin. This class does implement ConfigurationSerializable, which means
 * that you can use it to save skins in config, but do however note that for the class to be registered correctly, you
 * should always call NameTagChanger.INSTANCE.enable() in your onEnable() (not before checking if it is already enabled,
 * of course) and call NameTagChanger.INSTANCE.disable() in your onDisable (and again, check if NameTagChanger is
 * already disabled first).
 *
 * @author AlvinB
 */
class Skin implements ConfigurationSerializable
{
     ...
     * <p>
     * Since fetching this skin requires making asynchronous requests to Mojang's servers, a call back mechanism using
     * the SkinCallBack class is implemented. This call back allows you to also handle any errors that might have
     * occurred while fetching the skin. If no users with the specified username can be found, the skin passed to the
     * callback will be Skin.EMPTY_SKIN.
     * <p>
     ...
     * <p>
     * Since fetching this skin might require making asynchronous requests to Mojang's servers, a call back mechanism
     * using the SkinCallBack class is implemented. This call back allows you to also handle any errors that might have
     * occurred while fetching the skin.
     * <p>
     * The call back will always be fired on the main thread.
     *
...
/**
 * Implementation to make requests to Mojang's API servers. See http://wiki.vg/Mojang_API for more information.
 * <p>
 * Since all of these methods require connections to Mojang's servers, all of them execute asynchronously, and do
 * therefor not return any values. Instead, a callback mechanism is implemented, which allows for processing of data
 * returned from these requests. If an error occurs when retrieving the data, the 'successful' boolean in the callback
 * will be set to false. In these cases, null will be passed to the callback, even if some data has been received.
 * <p>
 * Each method has an synchronous and an asynchronous version. It is recommended that you use the synchronous version
 * unless you're intending to do more tasks that should be executed asynchronously.
 *
 * @author AlvinB
 */
class MojangAPIUtil
{
     ...
     * <p>
     * The plugin instance in the same jar as this class should automatically be found, so only use this if you for
     * whatever reason need to use another plugin instance.
     *
     ...
     * Statuses can be either RED (meaning service unavailable), YELLOW (meaning service available, but with some
     ...
     * <p>
     * The timestamp is in UNIX Time, and if -1 is used as the timestamp, it will get the current user who has this
     * name.
     * <p>
     * The callback contains the UUID and the current username of the UUID. If the username was not occupied at the
     * specified time, the next person to occupy the name will be returned, provided that the name has been changed away
     * from at least once or is legacy. If the name hasn't been changed away from and is not legacy, the value passed to
     ...
     * <p>
     * The callback is passed a Map<String, Long>, the String being the name, and the long being the UNIX millisecond
     * timestamp the user changed to that name. If the name was the original name of the user, the long will be -1L.
     * <p>
    ...
    /**
     * Same as #getUUIDWithCallBack but is entirely executed on the current thread. Should be used with caution to avoid
    ...
    /**
     * Same as #getSkinDataWithCallBack but is entirely executed on the current thread. Should be used with caution to
     * avoid blocking any important activities on the current thread.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Gets the Skin data for a certain user. If the user cannot be found, the value passed to the callback will be
     * null.
     *
    ...
    public static class SkinData
    {
        ...
        public boolean equals(Object obj)
        {
            ...
            return this.uuid.equals(skinData.uuid) && this.name.equals(skinData.name) && (this.skinURL == null ? skinData.skinURL == null : this.skinURL.equals(skinData.skinURL)) && (this.capeURL == null ? skinData.capeURL == null : this.capeURL.equals(skinData.skinURL)) && this.timeStamp == skinData.timeStamp && this.base64.equals(skinData.base64)
                    && (this.signedBase64 == null ? skinData.signedBase64 == null : this.signedBase64.equals(skinData.signedBase64));
        }
     ...
     * <p>
     * Once some data is received (or an error is thrown) the callBack method is fired with the following data:
     * <p>
     ...
     * <p>
     * This interface is annotated with @FunctionalInterface, which allows for instantiation using lambda expressions.
     */
    @FunctionalInterface
